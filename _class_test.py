...
class GameScreen(Screen):
    ...
    def init_canvas(self):
        ...
        with self.canvas:
            ...
            self.obstacle_color = Color(1, 1, 1, 0.5)
            self.obstacle_rect = RoundedRectangle(pos=(25, -15), size=(WIDTH - 50, 15), radius = [10] * 4)
    ...
    def load_level(self):
        ...
        self.target_rect.pos = level_data[str(self.level)]["target_rect"]
        
        if "obstacle_rect" in level_data[str(self.level)]:
            self.obstacle_rect.pos = level_data[str(self.level)]["obstacle_rect"]
        else: 
            self.obstacle_rect.pos = (25, -15)
    ...
    def update(self, dt):
        ...
        self.obstacle_color.a = 1 if self.ground_touches == 1 else 0.5
        self.height_color.a = max(0, self.height_color.a - (1/120))

        # Check for collision with the obstacle
        if self.obstacle_color.a == 1:
            obstacle_top = self.obstacle_rect.pos[1] + 15
            obstacle_bottom = self.obstacle_rect.pos[1]

            if self.ball_y < obstacle_top and self.ball_y + 25 > obstacle_bottom and self.ball_velocity > 0:
